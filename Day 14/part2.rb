...
#DEBUG
def draw_grid(grid)
    for i in $window_miny..$window_maxy+3#for i in 0..grid.size - 1
        for j in $window_minx - 65..$window_maxx + 65#for j in 0..grid[0].size - 1
            print grid[i][j]
        end
    puts ''
    end
...
input.each do |line|
    ...
    for i in 0...raw.size - 1
        ...
        if pt1[0] < $window_minx || pt2[0] < $window_minx
            $window_minx = pt1[0] < pt2[0] ? pt1[0] : pt2[0]
        end
        if pt1[0] > $window_maxx || pt2[0] > $window_maxx
            $window_maxx = pt1[0] < pt2[0] ? pt1[0] : pt2[0]
        end
...
draw_line(grid,[0,$window_maxy + 2],[grid[0].size-1,$window_maxy + 2])
